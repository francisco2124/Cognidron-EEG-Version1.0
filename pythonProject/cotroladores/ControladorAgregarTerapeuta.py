...
class Controlador_AgrgarTerapeutas(QtWidgets.QMainWindow):
    ...
    def inicializarGUI(self):
        ...
        self.ui.btnVerPas_1.pressed.connect(self.visualizarPass)
        self.ui.btnVerPas_1.released.connect(self.ocultarPass)
        self.ui.btnVerPas_2.pressed.connect(self.visualizarPass2)
        self.ui.btnVerPas_2.released.connect(self.ocultarPass2)
    ...
    #Funciones para visualizar y ocultar la contraseñ
    def visualizarPass(self):

        self.ui.lePassword.setEchoMode(QtWidgets.QLineEdit.Normal)

    def ocultarPass(self):

        self.ui.lePassword.setEchoMode(QtWidgets.QLineEdit.Password)

    def visualizarPass2(self):

        self.ui.leCofirnarPassword.setEchoMode(QtWidgets.QLineEdit.Normal)

    def ocultarPass2(self):

        self.ui.leCofirnarPassword.setEchoMode(QtWidgets.QLineEdit.Password)
   ...
   #---------------------------------------------------------------
    ...
    #--------------------------------------------------------------------
        ...
        password = self.ui.lePassword.text().strip()
        confirPaswword = self.ui.leCofirnarPassword.text().strip()
        user = self.ui.leNombreUsuario.text().strip()
        ...
        #Continuar validadno campos obligatorios
        ...
        elif len(user) == 0 or user.isalnum() == False:
            Alerta = QMessageBox.information(self, 'Alerta', "Ingresa un Nombre de usuario usando solo letras y numeros", QMessageBox.Ok)
        elif len(password) == 0 or password.isalnum() == False:
            Alerta = QMessageBox.information(self, 'Alerta', "Ingresa la contraseña usando solo letras y numero", QMessageBox.Ok)
        elif password != confirPaswword:
        ...
        elif validrSiesNum(password) == False or password.isalpha() == True:
            Alerta = QMessageBox.information(self, 'Alerta', "La contraseña debe tener letras y un numero", QMessageBox.Ok)
            ...
            if resuld == True:
                ...
                Alerta = QMessageBox.information(self, 'Alerta', "El nombre de usuario ya existe", QMessageBox.Ok)
                ...
                self.modelo.agregarUsario(user, password, admin)
                self.modelo.agregar(app, apm, nombre,genero, date, codPostal,localidad, calle, num,
                                nacionalidad, contacto, correoElec, borradoLogico, idMunicipio[0], user)
    ...
    def limpiarCampos(self):
        ...
        self.ui.leNombreUsuario.setText("")
        self.ui.lePassword.setText("")
        self.ui.leCofirnarPassword.setText("")
