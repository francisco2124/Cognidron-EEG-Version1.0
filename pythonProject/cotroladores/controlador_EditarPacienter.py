...
from vistas.editarPaciente import Ui_EditarPaciente
...
class Control_EditarPaciente(QtWidgets.QMainWindow):

    def __init__(self, idPaciente):
        super().__init__()
        print("soy la vista de editar un paciente")
        self.ui= Ui_EditarPaciente()
        self.ui.setupUi(self)
        self.idPacinte = idPaciente
    ...
    def editar(self):
        ...
        if len(app) >= 1 and validarCamposConEspaciosNoObligatorios(app):
            Alerta = QMessageBox.information(self, 'Alerta', "Ingresa el Apellido Paterno usando letras y espacios", QMessageBox.Ok)

        elif len(apm) >= 1 and validarCamposConEspaciosNoObligatorios(apm):
            Alerta = QMessageBox.information(self, 'Alerta', "Ingresa el Apellido Materno letras y espacios", QMessageBox.Ok)
        ...
        #Continuar validadno campos obligatorios
            ...
            Alerta = QMessageBox.information(self, 'Confirmacion', "Se edito el paciente", QMessageBox.Ok)
            self.cargarPlaceHolder()
    ...
    def cargarPlaceHolder(self):
        ...
        self.ui.leNacionalidad.setPlaceholderText(str(ListaDatos[10]).strip())
        ...
        self.ui.leCodigoPostal.setPlaceholderText(str(ListaDatos[6]).strip())
        self.ui.leNumeroContacto.setPlaceholderText(str(ListaDatos[12]).strip())
        self.ui.leCorreoElectronico.setPlaceholderText(str(ListaDatos[13]).strip())
